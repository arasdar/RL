...
class Agent():
    ...
    def learn(self, E, gamma):
        ...
        Q2values = self.q_target(S2).detach().max(1)[0].unsqueeze(1)
        # Compute Q target for current states (S)
        Q2values_target = rewards + (gamma * Q2values * (1 - dones))

        # Get expected Q (values) from local model
        Qvalues = self.q(S).gather(1, A)

        # Compute loss
        #loss = F.mse_loss(Q_expected, Q_targets)
        loss = ((Qvalues - Q2values_target)**2).mean()
        # Minimize the loss
